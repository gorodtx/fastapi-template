...
class ResponseMapper:
    __slots__: tuple[str, ...] = ("_rules",)

    def __init__(self: ResponseMapper) -> None:
        self._rules: dict[
    ...
    def register[S, D](
        ...
        def _wrapper(value: object) -> object:
            if not isinstance(value, src_type):
                raise TypeError(
                    f"Expected {src_type.__name__}, got {type(value).__name__}"
                )
            return fn(value)
    ...
    def present[D](self: ResponseMapper, src: object, dto_type: type[D]) -> D:
        key = (type(src), dto_type)
        if key not in self._rules:
            raise KeyError(
                f"No mapper registered for {type(src).__name__} -> {dto_type.__name__}"
            )
