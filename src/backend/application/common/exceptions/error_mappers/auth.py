...
def map_invalid_credentials() -> Callable[[Exception], AppError]:
    return map_auth_error((ValueError, unauthenticated("Invalid email or password")))


def map_refresh_replay() -> Callable[[Exception], AppError]:
    return map_auth_error((PermissionError, unauthenticated("Refresh token replay detected")))


def map_invalid_refresh() -> Callable[[Exception], AppError]:
    return map_auth_error((PermissionError, unauthenticated("Invalid refresh token")))
