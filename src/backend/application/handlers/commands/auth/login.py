...
from backend.application.common.interfaces.auth.ports import JwtIssuer, RefreshStore
    ...
    async def __call__(
        ...
        if user_result.is_err():
            ...
            if isinstance(err, NotFoundStorageError):
                return ResultImpl.err(UnauthenticatedError("Invalid email or password"))
            return ResultImpl.err(map_storage_error_to_app()(err))
        ...
        def verify_password() -> Awaitable[bool]:
            return self.password_hasher.verify(cmd.raw_password, user.password.value)

        verify_result = await capture_async(verify_password, map_invalid_credentials())
