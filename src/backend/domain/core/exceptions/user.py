...
class InsufficientPermissionsError(UserPermissionError):
    __slots__: tuple[str, ...] = ("_required_permission", "_username")
...
class RoleAssignmentNotAllowedError(UserPermissionError):
    __slots__: tuple[str, ...] = ("_reason", "_role", "_username")
...
class SelfOperationNotAllowedError(UserPermissionError):
    __slots__: tuple[str, ...] = ("_operation",)

    def __init__(self: SelfOperationNotAllowedError, operation: str) -> None:
        self._operation = operation
...
class UserDataCorruptedError(CorruptedInvariantError):
    ...
    __slots__: tuple[str, ...] = ("_details", "_user_id")

    def __init__(
        self: UserDataCorruptedError, user_id: UUID, details: str
    ) -> None:
        self._user_id = user_id
        self._details = details
        super().__init__(
            f"CRITICAL: User data corrupted for user {user_id!s}: {details}"
        )
...
class DuplicateUserIdError(CorruptedInvariantError):
    ...
    def __init__(self: DuplicateUserIdError, user_id: UUID) -> None:
        self._user_id = user_id
        super().__init__(
            f"CRITICAL: Multiple users found with same ID {user_id!s}"
        )
...
class UserInvariantViolationError(CorruptedInvariantError):
    ...
    __slots__: tuple[str, ...] = ("_invariant", "_user_id")

    def __init__(
        self: UserInvariantViolationError, user_id: UUID, invariant: str
    ) -> None:
        self._user_id = user_id
        self._invariant = invariant
        super().__init__(
            f"CRITICAL: User {user_id!s} invariant violated: {invariant}"
        )
